...
# Worktree-backed tmux sessions with tracked Claude sessions — one session per
# branch, worktrees under <repo>/.claude/worktrees/. Thin wrapper around the
# implementation in scripts/cw/. See `cw -h` and docs/worktrees.md.
#
#   cw <branch> [note…]   Create/attach the session for <branch>
#   cw c [label]          Spawn another Claude session in this worktree
#   cw note [text|--gen]  Show/set/haiku-generate the worktree's purpose note
#   cw ls | rm <branch> | board
cw() {
    command bash "${DOTFILES_DIR:-$HOME/.dotfiles}/scripts/cw/cw" "$@"
}
